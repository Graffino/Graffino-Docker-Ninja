...
const promptUserForInfo = async () => {
  ...
  const projectfolderName = pathComponents[pathComponents.length - 2].toLocaleLowerCase()

  const generalQuestions = [{
    type: 'text',
    name: 'name',
    initial: projectfolderName,
    format: val => val.replace(/^[.,_]/, '').replace(/[ ]/g, '-').toLowerCase(), // format required by package.json
  ...
  },
  {
    ...
    name: 'title',
    initial: (prev, values) => {
      return `${capitalize(values.name.replace(/^[^a-zA-Z]/, '').replace(/[^a-zA-Z]$/, '').replace(/[-]/g, ' '))}`
    },
  ...
  await fs.writeJson(path.resolve(__dirname, '../package.json'), packageJson, { spaces: 2 })
...
const setUpEnv = async () => {
  const envQuestions = [
    ...
    },
    {
      ...
      name: 'THEME_URL',
      initial: (prev, values) => {
        return `https://${values.THEME_NAME.replace(/[ ]/g, '-').toLowerCase()}.graffino.dev`
      },
    ...
    },
    {
      type: 'text',
      name: 'PLUGIN_WPML_ID',
      message: 'Multi Language Wordpress ID:'
    },
    {
      type: 'text',
